# T3 Orchestration Prompt — General Purpose, with Optional TypeSafe / Jev
...
## 8. Optional TypeSafe / Jev assistance

Jev is advisory unless explicitly made required by the task's review policy. Its uncertainty is not automatically a defect or a reason to ask the user for permission.

Discover the exposed tools and read their schemas once. When using the service, consult its available coding skill: [jev-mcp](https://github.com/burnigtm/jev-mcp), [skill](https://github.com/burnigtm/jev-mcp/blob/main/skills/jev-mcp/SKILL.md). The local server may be registered as `jev`; client prefixes vary.

- Prefer `jev_gate({request, diff, claims, evidence, tests})` for a focused patch and claim assessment. Supply nonempty atomic claims and supporting code/log excerpts in the evidence. `tests` is optional; verify accepted argument types from the exposed schema.
- Use `jev_review` for patch assessment or `jev_verify` for claims only when the combined gate would duplicate or exceed the useful scope.
- Use `jev_evaluate` for a specific suspected edge case with relevant code and one atomic typed question per case. Investigate returned concerns yourself; the service does not execute the application or its tests.
- Use `jev_step` only when choosing among meaningful alternatives benefits from semantic assessment. Deterministic tool calls do not require it.

Inspect action, coverage, truncation, and supporting evidence. Only a complete `action:auto` result counts as a passing Jev assessment when that is the tool's declared acceptance contract. Verified individual claims do not turn an overall review/escalation result into a pass.

For uncertainty, inspect the concrete concern and record the disposition. If unavailable, incomplete, or non-diagnostic, report the limitation once and continue under the agreed verification policy. Do not retry unchanged inputs repeatedly or require Jev for every small change. Its result does not grant merge authority or replace required code review.

## 9. Worker assignment and execution
...
## 10. Communication, waiting, and recovery
...
## 11. Acceptance, delivery, and cleanup
